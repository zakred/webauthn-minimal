...
class Wso2Service {
    ...
    base64UrlToBase64 = (base64Url) => base64Url.replace(/\-/g, '+').replace(/_/g, '/') + '='.repeat((base64Url.length % 4) ? 4 - (base64Url.length % 4) : 0);
    #handleHttpError(e) {
        if (e.response) {
    ...
    startAuthentication = async() => {
        ...
        try {
            ...
            if (resp.status === 302) {
                const queryParams = url.parse(resp.headers.location, true).query;
                const data = JSON.parse(queryParams.data);
                data.sessionDataKey = queryParams.sessionDataKey
                data.publicKeyCredentialRequestOptions.challenge = this.base64UrlToBase64(data.publicKeyCredentialRequestOptions.challenge);
                return data;
            }
    ...
    commonAuth = async (req) => {
        ...
        try {
            return await axios.post(this.WSO2_COMMON_AUTH_URL, formData,  config);
        } catch (e) {
            const resp = e.response;
            if (resp.status === 302) {
    ...
    getTokens = async (authorization_code) => {
        ...
        try {
            const resp = await axios.post(this.WSO2_TOKEN_URL, formData,  config);
            return resp.data;
        } catch (e) {
            return this.#handleHttpError(e);
        }
